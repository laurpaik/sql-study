...
UPDATE mytable
  ...
  WHERE --specify where you're setting these variables
  ;
```
...
```sql
DELETE FROM mytable WHERE --something that specifies which line to delete
--probably a column header, like WHERE city = 'Hayward' will delete the row on Hayward
```
